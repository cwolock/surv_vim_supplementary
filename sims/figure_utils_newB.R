...
make_sim_plot <- function(summ, scenario, big = TRUE, wd, fname){
  ...
  if (!big){
    ...
    bias_plot_1 <- plot_tib %>% filter(indx == "(a)~X[1]~(non-null)") %>%
      ...
      scale_y_continuous(
        # limits = c(this_bias_lim_lower, this_bias_lim_upper),
    ...
    var_plot_1 <- plot_tib  %>% filter(indx == "(a)~X[1]~(non-null)") %>%
      ...
      xlab(xlab) +
      scale_y_continuous(
        # limits = c(this_var_lim_lower, ifelse(Switch, NA, this_var_lim_upper)),
        limits = c(0.25, 2),
        labels = scales::number_format(accuracy = 0.01,
                                       decimal.mark = '.')) +
      theme(#axis.ticks.length.x = unit(0, "cm"),
    ...
    width_plot_1 <-plot_tib %>% filter(indx == "(a)~X[1]~(non-null)") %>%
      ...
      xlab(xlab) +
      scale_y_continuous(
        # limits = c(this_width_lim_lower, this_width_lim_upper),
        labels = scales::number_format(accuracy = 0.01,
                                       decimal.mark = '.')) +
      theme(#axis.ticks.length.x = unit(0, "cm"),
        # axis.text.x = element_blank(),
    ...
    power_plot_6 <- plot_tib %>% filter(indx == "(b)~X[6]~(null)") %>%
      ...
      theme_bw() +
      scale_y_continuous(
        # limits = c(this_power_lim_lower, this_power_lim_upper),
        labels = scales::number_format(accuracy = 0.01,
                                       decimal.mark = '.')) +
      theme(#axis.ticks.length.x = unit(0, "cm"),
        # strip.background = element_blank(),
