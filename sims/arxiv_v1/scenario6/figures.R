...
dat <- readRDS("C:/Users/cwolo/Dropbox/UW/DISSERTATION/surv_vim_supplementary/scratch/scenario6.rds")
...
fills <- c("DR pseudo-outcome" = "grey33", "Conditional surv. function" = "grey75")
p <- dat %>%
  ggplot() +
  geom_histogram(aes(x = robust_err, fill = "DR pseudo-outcome", y = after_stat(count / sum(count))),
                 bins = 50) +
  geom_histogram(aes(x = nonrobust_err, fill = "Conditional surv. function", y = -after_stat(count / sum(count))),
                 bins = 50) +
  scale_fill_manual(values = fills, breaks=c("DR pseudo-outcome", "Conditional surv. function")) +
  ...
  xlab("Error") +
  ylab("Frequency") +
  scale_y_continuous(breaks = c(-0.03, -0.02, -0.01 ,0,0.01),
                     labels = c("0.03", "0.02", "0.01", "0", "0.01"),
...
ggsave(filename = "C:/Users/cwolo/Dropbox/UW/DISSERTATION/surv_vim_supplementary/scratch/scenario6.pdf",
       plot = p,
       device = "pdf",
       width = 8,
       height = 8,
