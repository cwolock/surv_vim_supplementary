...
multisplit <- function(f, f1, method, this_vim = NULL){
  ...
  for (i in 1:n3){
    ...
    for (j in 1:n2){
      for (k in 1:n1){
        z <- (dat$est[k] - ci_grid[j]) / sqrt(dat$var_est[k])
  ...
  for (k in 1:n1){
    ...
    pvals[k] <- curr_ps_twosided[ceiling(n2/2)]
  ...
  f_avg <- f %>% group_by(landmark_time, indx_name, vim) %>% summarize(est = mean(est))

  combined_ss <- f1 %>% select(landmark_time, vim, indx, indx_name) %>%
    mutate(cil = cil, ciu = ciu, pval = pvals)

  combined_ss <- left_join(combined_ss, f_avg, by = c("landmark_time", "indx_name", "vim"))
...
f_fem <- readRDS("female_analysis_interactions_oldlib_smallersigma_subsample1500.rds")
names(f_fem)[names(f_fem) == "tau"] <- "landmark_time"
f_fem <- f_fem %>% filter(approach == "marginal")  %>% mutate(n_eff = 1846,
                                                              var_est = var_est/n_eff)
setwd("/Users/cwolock/Dropbox/UW/RESEARCH/paper_supplements/surv_vim_supplementary/data_analysis/male")
f_mal <- readRDS("male_analysis_interactions_oldlib_smallersigma_subsample1500_oldseed.rds")
names(f_mal)[names(f_mal) == "tau"] <- "landmark_time"
f_mal <- f_mal %>% filter(approach == "marginal") %>% mutate(n_eff = 795,
                                                             var_est = var_est/n_eff)
f <- f_fem
f$est <- f_mal$est - f_fem$est
f$var_est <- f_mal$var_est + f_fem$var_est

f1 <- f[f$seed == unique(f$seed)[1],]
ss_both_marginal_diff <- multisplit(f, f1, method)
